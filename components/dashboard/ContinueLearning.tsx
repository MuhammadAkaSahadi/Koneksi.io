...
export function ContinueLearning({ latestProgress }: ContinueLearningProps) {
  ...
  if (!latestProgress) {
    return (
      <div className="bg-white rounded-2xl border border-slate-200 p-8 sm:p-12 text-center shadow-[0_1px_3px_rgba(0,0,0,0.05)] flex flex-col items-center justify-center min-h-[240px] transition-all">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center border-b border-slate-200 pb-2">
        <button className="text-sm font-bold text-slate-800 relative px-2 pb-2.5 after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-[#0891b2] cursor-default">
          Course
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        <Link 
          ...
          className="group block bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] hover:border-slate-350 transition-all duration-300 transform hover:-translate-y-1"
        >
          {/* Card Thumbnail */}
          ...
          <div className="p-5 flex flex-col gap-3.5">
            ...
            {/* Title */}
            <h3 className="text-[15px] font-bold text-slate-800 font-heading leading-snug group-hover:text-[#0891b2] transition-colors line-clamp-2 min-h-[44px]">
              {latestProgress.themeTitle}
            </h3>

            {/* Last read status */}
            <p className="text-xs text-slate-400 font-semibold truncate">
              Last read: <span className="text-slate-700">{latestProgress.lessonTitle}</span>
            </p>
